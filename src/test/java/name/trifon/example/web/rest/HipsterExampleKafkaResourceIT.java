...
import name.trifon.example.config.KafkaProperties;
...
class HipsterExampleKafkaResourceIT {
    ...
    void setup() {
        ...
        HipsterExampleKafkaResource kafkaResource = new HipsterExampleKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/hipster-example-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/hipster-example-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
